...
def main():
    clear()
    setTitle(f"@Zeldris Menu v{THIS_VERSION}")
    astraahometitle()
...
\n                                      {y}[{w}23{y}]{w} Remover                 {y}[{w}25{y}]{w} Exit\n\n\n\n\n\n\n\n\n\n                                                                                                     {y}[{b}<{y}]{w} Previous Page""")
        ...
        if choice == '21':
            ...
            transition()
            astraahometitle()
            Anime.Fade(Center.Center(bannere), Colors.purple_to_blue, Colorate.Vertical, time=5)
...
if __name__ == "__main__":
    import sys
    setTitle("@Zeldris Premium Loading...")
    
    System.Size(120, 30)
    Anime.Fade(Center.Center(banner), Colors.blue_to_red, Colorate.Diagonal, time=3)
    Anime.Fade(Center.Center(loading), Colors.purple_to_blue, Colorate.Diagonal, time=2)
    ...
    try:
        ...
        input(f"{y}[{Fore.RED}#{y}]{w} Sorry but, your python version ({sys.version_info[0]}.{sys.version_info[1]}.{sys.version_info[2]}) is not compatible with @Zeldris, please download python 3.9 or higher.")
        sys.exit()
    else:
        search_for_updates()
        if not os.path.exists(getTempDir()+"\\atio_proxies"):
